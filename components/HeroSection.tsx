  ...
  return (
    <section
      ...
      <div className="absolute inset-0 z-0">
        ...
        {/* Strong left-side gradient so white text is always readable over the photo */}
      ...
      <div className="absolute bottom-24 left-1/2 -translate-x-1/2 z-20 flex gap-2">
        ...
        <button
          ...
          className={`h-1.5 rounded-full transition-all duration-300 ${i === activeSlide ? 'w-8 bg-accent' : 'w-3 bg-white/30'}`}
      ...
      <div className="absolute top-0 right-0 z-20 hidden lg:block">
        <div className="relative w-[42%] h-screen">
          ...
          {/* AUM Card */}
          <div className="absolute top-56 right-16 glass-card rounded-lg px-5 py-4 animate-float-slow" style={{ animationDelay: '1.5s' }}>
            <p className="text-white/50 text-xs font-500 uppercase tracking-wider mb-1">Portfolio Assets Management</p>
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-6 pb-16 pt-36">
        <div className="max-w-3xl">
          ...
          <div className="inline-flex items-center gap-2 px-3 py-1.5 border border-white/20 rounded-sm mb-6 backdrop-blur-sm">
            ...
            <span className="text-xs font-600 uppercase tracking-widest text-white/80">Global Financial Services</span>
          </div>

          <h1 className="font-display font-800 text-white leading-[0.88] tracking-tighter mb-6"
          style={{ fontSize: 'clamp(3rem, 8vw, 6.5rem)' }}>
            UNLOCKING<br />
            <span className="text-gradient-sky">GLOBAL</span><br />
            INVESTMENTS
          </h1>
          ...
          <div className="mt-14 pt-8 border-t border-white/10">
            <p className="text-xs font-600 uppercase tracking-widest text-white/30 mb-5">Trusted by leading institutions</p>
            <div className="flex flex-wrap items-center gap-8">
              {['BlackRock', 'Vanguard', 'Fidelity', 'J.P. Morgan']?.map((name) =>
              <span key={name} className="text-sm font-700 uppercase tracking-wider text-white/25 hover:text-white/50 transition-colors cursor-default">
                  {name}
                </span>
              )}
            </div>
